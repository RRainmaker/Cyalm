...
class Errors(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            return await ctx.send('You are not allowed to use that command in a DM')

        # handle NoPrivateMessage first as it inherits from CheckFailure, then consume the other checks
        if ctx.cog and ctx.cog.qualified_name == 'Music' and isinstance(error, commands.CheckFailure):
            return
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(f'I seem to be missing the `{error.param.name}` input in the {ctx.command.name} command')
